...
static sf32_rcc_gate_t sf32_rcc_resolve(sf32_rcc_module_t module)
{
    ...
    case SF32_RCC_MOD_PINMUX1:
        return (sf32_rcc_gate_t){&HPSYS_RCC->ENR1.R, &HPSYS_RCC->ESR1.R,
                                 &HPSYS_RCC->ECR1.R, &HPSYS_RCC->RSTR1.R,
    ...
    case SF32_RCC_MOD_GPIO1:
        return (sf32_rcc_gate_t){&HPSYS_RCC->ENR2.R, &HPSYS_RCC->ESR2.R,
                                 &HPSYS_RCC->ECR2.R, &HPSYS_RCC->RSTR2.R,
...
void sf32_rcc_select_clock(sf32_rcc_clock_module_t module, uint32_t source)
{
    ...
    uint32_t value = HPSYS_RCC->CSR.R;

    value &= ~mask;
    value |= (source & width) << shift;
    HPSYS_RCC->CSR.R = value;
}
